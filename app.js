...
function syncLocalColors(){
  // selaraskan simpanan warna peranti dengan warna semasa (dipanggil selepas terbit berjaya)
  try{
    if(localStorage.getItem(COLOR_KEY)!==null){
      const m={};
      for(const t of state.config.teachers) m[t.name]=t.color;
...
const ui = { tab:(typeof window!=='undefined'&&window.innerWidth<640)?'senarai':'guru', tetapanGuru:0, busy:false, admin:false };
...
document.addEventListener('click', e=>{
  ...
  else if(act==='save-colors'){
    try{
      const m={};
      for(const g of cfg.teachers) m[g.name]=g.color;
      localStorage.setItem(COLOR_KEY, JSON.stringify(m));
      toast('💾 Warna disimpan dalam pelayar ini — akan dipakai secara automatik setiap kali dibuka di sini.');
    }catch(e){
      toast('Simpanan pelayar disekat dalam panel ini — buka fail HTML/laman web untuk guna ciri ini, atau guna Eksport JSON.', true);
    }
    return;
  }
  else if(act==='clear-colors'){
    try{
